...
const businessKinds: Array<{
  ...
  { key: "teaching", label: "教学记录", endpoint: "/admin/business/teaching-records" },
  { key: "growth", label: "成长反馈", endpoint: "/admin/business/growth-records" },
  ...
  { key: "research", label: "教研活动", endpoint: "/admin/business/research-activities" },
];
...
function recordScope(record: BusinessRecord) {
  if (record.student) {
    return `${record.student.name} / ${record.student.class?.name ?? "未分班"}`;
  }
...
function recordStatus(kind: BusinessKind, record: BusinessRecord) {
  if (kind === "growth") return record.visibleToParent ? "家长可见" : "仅内部可见";
  ...
  if (kind === "workflow") {
    const checked = record.steps?.filter((item: BusinessRecord) => item.checked).length;
...
export function BusinessPanel({
  ...
  const [result, setResult] = useState<PageResult>({ items: [], total: 0, page: 1, pageSize: 10 });
  ...
  const columns = [
    ...
    { title: "班级 / 学生", key: "scope", render: (_: unknown, record: BusinessRecord) => recordScope(record) },
    { title: "教师", key: "owner", render: (_: unknown, record: BusinessRecord) => recordOwner(record) },
    {
      title: "状态",
      key: "status",
      render: (_: unknown, record: BusinessRecord) => (
    ...
    { title: "时间", key: "time", render: (_: unknown, record: BusinessRecord) => formatDate(recordTime(kind, record)) },
    {
      title: "操作",
      key: "actions",
      render: (_: unknown, record: BusinessRecord) => (
        <Space>
          ...
          {kind === "lesson" || kind === "research" ? (
            <Select
              ...
              value={record.status}
              style={{ width: 100 }}
              options={statusOptions[kind]?.map((value) => ({ value, label: statusLabels[value] ?? value }))}
  ...
  const showStudent = !["teaching", "workflow", "lesson", "research"].includes(kind);
  ...
  return (
    <Space direction="vertical" size="middle" style={{ width: "100%" }}>
      <Card>
        ...
        >
          <Form.Item name="classId" label="班级">
            <Select allowClear showSearch optionFilterProp="label" style={{ width: 150 }} options={classes.map((item) => ({ value: item.id, label: item.name }))} />
          </Form.Item>
          <Form.Item name="teacherId" label="教师">
            <Select allowClear showSearch optionFilterProp="label" style={{ width: 150 }} options={teachers.map((item) => ({ value: item.id, label: item.name }))} />
          </Form.Item>
          {showStudent ? (
            <Form.Item name="studentId" label="学生">
              <Select allowClear showSearch optionFilterProp="label" style={{ width: 150 }} options={visibleStudents.map((item) => ({ value: item.id, label: item.name }))} />
            </Form.Item>
          ) : null}
          {statusOptions[kind] ? (
            <Form.Item name="status" label="状态">
              <Select allowClear style={{ width: 130 }} options={statusOptions[kind]?.map((value) => ({ value, label: statusLabels[value] ?? value }))} />
            </Form.Item>
          ) : null}
          {typeOptions[kind] ? (
            <Form.Item name="type" label="类型">
              <Select allowClear style={{ width: 130 }} options={typeOptions[kind]?.map((value) => ({ value, label: typeLabels[value] ?? value }))} />
            </Form.Item>
          ) : null}
          <Form.Item name="from" label="开始"><Input type="date" style={{ width: 145 }} /></Form.Item>
          <Form.Item name="to" label="结束"><Input type="date" style={{ width: 145 }} /></Form.Item>
          <Form.Item>
            <Space>
      ...
      <Drawer title="业务记录详情" width={560} open={Boolean(selected)} onClose={() => setSelected(null)}>
        {selected ? (
          <Descriptions bordered column={1} size="small">
            <Descriptions.Item label="记录名称">{recordTitle(kind, selected)}</Descriptions.Item>
            <Descriptions.Item label="班级 / 学生">{recordScope(selected)}</Descriptions.Item>
            <Descriptions.Item label="教师">{recordOwner(selected)}</Descriptions.Item>
            <Descriptions.Item label="状态">{recordStatus(kind, selected)}</Descriptions.Item>
            <Descriptions.Item label="业务时间">{formatDate(recordTime(kind, selected))}</Descriptions.Item>
            <Descriptions.Item label="内容">{selected.content ?? selected.description ?? selected.objectives ?? "-"}</Descriptions.Item>
            <Descriptions.Item label="记录编号">{selected.id}</Descriptions.Item>
          </Descriptions>
        ) : null}
      </Drawer>
    </Space>
