...
const TOKEN_KEY = `teacherToken:${AUTH_MODE}`;
const BINDING_TOKEN_KEY = "teacherWechatBindingToken";
...
async function wechatLogin() {
  ...
  const result = await publicRequest("/auth/wechat-login", {
    method: "POST",
    data: { code: login.code, role: "teacher" },
  });
  ...
  if (result.status === "binding_required" && result.bindingToken) {
    ...
    throw new Error("请先绑定教师手机号");
  }
  throw new Error("微信登录状态无效，请重试");
}

export async function bindTeacherWechatPhone(phoneCode) {
  ...
  const result = await publicRequest("/auth/bind-phone", {
    method: "POST",
    data: { bindingToken, phoneCode, role: "teacher" },
  });
...
export function hasTeacherBindingToken() {
  return Boolean(Taro.getStorageSync(BINDING_TOKEN_KEY));
}

export function resetTeacherWechatLogin() {
  Taro.removeStorageSync(BINDING_TOKEN_KEY);
...
export async function teacherRequest(path, options = {}) {
  ...
  if (!token) token = await teacherLogin();
...
async function devLogin() {
  const result = await publicRequest("/auth/dev-login", {
    method: "POST",
    data: { role: "teacher", phone: "13800000001" },
  });
  saveToken(result.token);
  return result.token;
}
