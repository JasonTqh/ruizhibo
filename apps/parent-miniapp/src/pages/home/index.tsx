...
export default function HomePage() {
  ...
  async function load() {
    ...
    try {
      ...
      const nextChild =
        nextChildren.find((item) => item.id === activeChildId) || nextChildren[0];
  ...
  const child = children.find((item) => item.id === activeChildId) || children[0];
  ...
  return h(
    ...
    h(
      ...
      h(
        View,
        { className: `parent-refresh${loading ? " parent-refresh--loading" : ""}`, onClick: load },
        h(Text, null, "↻"),
      ),
    ),
    error
      ? h(
          ...
          h(Text, null, error),
          h(Text, { className: "parent-home__retry", onClick: load }, "重新加载"),
        )
      : null,
    children.length > 1
      ? h(
    ...
    child
      ...
      : h(
          ...
          h(Text, { className: "parent-empty-card__title" }, loading ? "正在加载孩子信息…" : "尚未绑定孩子"),
          h(Text, { className: "parent-empty-card__hint" }, "请联系管理员完成家长与孩子的绑定。"),
        ),
    h(
      View,
      { className: "parent-quick-grid" },
      quickItem("▣", "作业中心", pendingHomework.length ? `${pendingHomework.length} 项待完成` : "查看全部作业", "green", () =>
        Taro.navigateTo({ url: "/pages/homework/index" }),
      ),
      quickItem("↗", "成长记录", `${records.length} 条最新动态`, "yellow", () =>
        Taro.switchTab({ url: "/pages/growth/index" }),
      ),
      quickItem("◇", "通知任务", pendingNotices.length ? `${pendingNotices.length} 项待确认` : "暂无待确认", "coral", () =>
        Taro.switchTab({ url: "/pages/messages/index" }),
      ),
      quickItem("◌", "家校沟通", unreadCount ? `${unreadCount} 条未读消息` : "联系班级老师", "blue", () =>
        Taro.switchTab({ url: "/pages/messages/index" }),
      ),
    ),
    ...
    h(
      ...
      pendingNotices.length
        ...
        : reminderRow("通知任务", "所有通知均已查看确认", "已处理", "green", () =>
            Taro.switchTab({ url: "/pages/messages/index" }),
          ),
      unreadCount
        ? reminderRow(
        ...
        ? records.slice(0, 3).map((record, index) =>
            h(
              ...
              h(
                ...
                h(Text, { className: "growth-preview__type" }, recordTypeText(record.type)),
                h(Text, { className: "growth-preview__title" }, record.title),
                h(Text, { className: "growth-preview__content" }, record.content),
              ),
              h(Text, { className: "growth-preview__time" }, shortDate(record.happenedAt)),
            ),
          )
        : h(
            View,
            ...
            h(Text, null, loading ? "正在加载今日成长…" : "今天暂无新的成长记录"),
          ),
    ),
  );
}
...
function quickItem(icon, title, description, tone, onClick) {
  return h(
    ...
    h(Text, { className: `parent-quick-item__icon parent-quick-item__icon--${tone}` }, icon),
...
function reminderRow(title, description, badge, tone, onClick) {
  return h(
    ...
    h(Text, { className: `reminder-row__badge reminder-row__badge--${tone}` }, badge),
  );
}
...
function recordIcon(type) {
  return { workflow: "✓", teacher_feedback: "♡", homework: "✎", attendance: "⌂" }[type] || "•";
}

function recordTone(type, index) {
  return { workflow: "green", teacher_feedback: "coral", homework: "yellow", attendance: "blue" }[type] || ["green", "blue", "yellow"][index % 3];
}
